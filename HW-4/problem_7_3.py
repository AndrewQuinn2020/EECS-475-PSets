...
three_class_dataset_path = os.path.join(datasets_dir, "three_class_data.csv")
...
if __name__ == "__main__":
    logger.info(
        "EECS 475 - Andrew Quinn - Problem 7.3 - 3-Class with Multiclass Perceptron"
    )
    logger.info("-" * (88 - 11))
    logger.info("The Three Class CSV data we're training on can be found directly at:")
    logger.info("\t\t{}".format(three_class_data_url))
    ...
    if not os.path.exists(three_class_dataset_path):
        logger.warning("3-class toy dataset is missing... Downloading.")
        with open(three_class_dataset_path, "wb") as f:
            f.write(requests.get(three_class_data_url, allow_redirects=True).content)

    # Load in data.
    data = np.loadtxt(three_class_dataset_path, delimiter=",")

    # get input and output of dataset
    x = data[:-1, :]
    y = data[-1:, :]

    assert np.shape(x) == (2, 30)
    assert np.shape(y) == (1, 30)
